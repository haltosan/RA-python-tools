...
PRIMA_CONFIG = r'V:\RA_work_folders\Ethan_Simmons\layoutParser\primaLayout\config.yaml'
PRIMA_MODEL = r'V:\RA_work_folders\Ethan_Simmons\layoutParser\primaLayout\model_final.pth'
...
LSR_CONFIG = r'V:\RA_work_folders\Ethan_Simmons\layoutParser\customPubLayout\config.yaml'
LSR_MODEL = r'V:\RA_work_folders\Ethan_Simmons\layoutParser\customPubLayout\model_final.pth'
...
IMAGE_QUALITY = 300
# PATH IS GONE, CAN'T CONVERT FROM PDF UNTIL POPPLER_PATH IS FIXED
poppler_path = r'V:\FHSS-JoePriceResearch\papers\current\SAT_yearbooks\poppler-20.12.1\Library\bin'
startPage, endPage = 0, 0
# tesseract path
pytesseract.pytesseract.tesseract_cmd = r'V:\RA_work_folders\Ethan_Simmons\Tesseract-OCR\tesseract.exe'
...
def fileSortScore(name):
    ...
    return [int(i) for i in re.findall('[0-9]+', name)[:4]]  # only keep track of at most 4 numbers within the string for scoring
...
def obitMain():
    ...
    #fileNames.sort(key = obitImageSortKey)  # files are in some random ordering from listdir (based on address)
